...
// TODO: file operations: create, delete, rename, move, copy
    ...
    {
        public record FileIndexEntry(string Name, string Path, string Extension, bool IsDirectory, string? ParentPath, int Depth);
    ...
    public class TabChunk
        ...
        public IReadOnlyCollection<FileIndexEntry> SnapshotIndex() => _fileIndex.Values;
    ...
    private static TabChunk ErrorTabChunk(string tabId, string errorMessage)
    {
            ...
            [EnumeratorCancellation] CancellationToken token)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                FileIndexEntry? dirEntry = null;
                if (depth == 0)
                {
                    dirEntry = new FileIndexEntry(current.Name, current.FullName, string.Empty, true,
